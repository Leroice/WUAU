...
// The active persona's bundle is read by the screens via usePersona().

export type Persona = {
  id: string;
  label: string;
  blurb: string;
  user: { firstName: string; lastName: string; initials: string; location: string };
  cardHolder: string;
  totalBalance: { amount: string; currency: string };
  accounts: typeof ACCOUNTS;
  contacts: typeof CONTACTS;
  transactions: typeof TRANSACTIONS;
  cardSpends: typeof CARD_SPENDS;
  paymentsContacts: typeof PAYMENTS_CONTACTS;
  paymentsUpcoming: typeof PAYMENTS_UPCOMING;
  paymentsRecent: typeof PAYMENTS_RECENT;
};
...
const PersonaCtx = createContext<Ctx>({ persona: SAM, setPersona: () => {} });

export function PersonaProvider({ children }: { children: React.ReactNode }) {
  const [persona, setP] = useState<Persona>(SAM);
  const setPersona = (id: string) => setP(PERSONAS.find((p) => p.id === id) ?? SAM);
